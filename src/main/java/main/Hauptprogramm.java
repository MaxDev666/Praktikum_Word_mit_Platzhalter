...
public class Hauptprogramm{
	 ...
	 * 
	 * public static String hinzufuegenVorlage(String documentpath) { String
	 * lesenEintragErfolgreich = null; String hinzufügenEintragErfolgreich = null;
	 * 
	 * File f = new File(documentpath); if (f.exists()) { // ID aus dem Dateinamen
	 * bilden String[] splitpath = documentpath.split(Pattern.quote("\\")); String
	 * id = splitpath[splitpath.length - 1]; id = id.split("[.]")[0];
	 * lesenEintragErfolgreich = ansehenVorlage(id); if (lesenEintragErfolgreich ==
	 * "Eintrag nicht vorhanden.") { hinzufügenEintragErfolgreich =
	 * DatenbankverbindungVorlage.addVorlage(id, documentpath); } else {
	 * hinzufügenEintragErfolgreich =
	 * "Eintrag bereits vorhanden, bitte ändern und nicht hinzufügen."; } } else {
	 * hinzufügenEintragErfolgreich = "Datei wurde nicht gefunden."; } return
	 * hinzufügenEintragErfolgreich; }
	 * 
	 * 
	 ...
	 * 
	 * public static String ansehenVorlage(String id) { String
	 * lesenEintragErfolgreich = null; try { lesenEintragErfolgreich =
	 * DatenbankverbindungVorlage.getVorlage(id); } catch (NoResultException e) {
	 * lesenEintragErfolgreich = "Eintrag nicht vorhanden."; } return
	 * lesenEintragErfolgreich; }
	 * 
	 * 
	 ...
	 * 
	 * public static String bearbeitenVorlage(String id, String newPath) { String
	 * rueckgabeAnMainMethode = null; String lesenEintragErfolgreich = null; if
	 * (newPath.trim().equals("")) { rueckgabeAnMainMethode = ansehenVorlage(id); }
	 * else { File f = new File(newPath); if (f.exists()) { lesenEintragErfolgreich
	 * = ansehenVorlage(id); if (lesenEintragErfolgreich ==
	 * "Eintrag nicht vorhanden.") { rueckgabeAnMainMethode =
	 * "Eintrag zum Ändern nicht vorhanden, bitte prüfen und eventuell hinzufügen.";
	 * } else { rueckgabeAnMainMethode =
	 * DatenbankverbindungVorlage.changeVorlage(id, newPath); } }else {
	 ...
	 * 
	 * String lesenEintragErfolgreich = ansehenVorlage(id); String
	 * rueckgabeAnMainMethode = null; if (lesenEintragErfolgreich !=
	 * "Eintrag nicht vorhanden.") { File vorlage = new
	 * File(lesenEintragErfolgreich); if (vorlage.delete()) { rueckgabeAnMainMethode
	 * = vorlage.getName() + " auf dem Dateisystem und in DB gelöscht.";
	 * DatenbankverbindungVorlage.deleteVorlage(id); } else { rueckgabeAnMainMethode
	 * = "Datei auf dem Dateisystem konnte nicht gelöscht werden."; } }else {
	 * rueckgabeAnMainMethode = lesenEintragErfolgreich; } return
	 * rueckgabeAnMainMethode; }
	 */
...
/*
	public static String replacePlaceholder(String id, String xmlfilepath) {
	  ...
	  try { 
		  ...
		  fehler = "Das fertige Word-Dokument konnte nicht gespeichert werden." +
				  e.toString(); 
	  } 
	  return fehler; */
}
